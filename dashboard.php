...
$result = $db->query("SELECT COUNT(*) as count FROM participants ");
if ($result) {
...
$result = $db->query("SELECT contribution FROM sponsors where sponsors_email = '$user_id'");
...
$result = $db->query('SELECT SUM(contribution) as contribution FROM sponsors');
if ($result) {
    if ($row = $result->fetch_assoc()) {
                        ...
                        <div class="overflow-hidden">
                            <div class="js-scrollbar pr-3" style="max-height: 250px;">
                                <ul class="list-unstyled u-indicator-vertical-dashed">
                                    <?php
                                    $result = $db->query('SELECT email, log_message, log_time FROM logs INNER JOIN login ON log_user = email ORDER BY log_time DESC');

                                    if ($result) {
                                        while ($row = $result->fetch_assoc()) { ?>
                                            <li class="media u-indicator-vertical-dashed-item">
                                                <span class="btn btn-xs btn-icon btn-primary rounded-circle mr-3">
                                                    <span class="btn-icon__inner"><?php echo $row['email'][0]; ?></span>
                                                </span>
                                                <div class="media-body">
                                                    <h5 class="my-1" style="font-size: 0.875rem;">
                                                        <?php echo $row['email']; ?></h5>
